...
extern "C" {
  ...
  void pitch_mark(float_list *input, int length, double samplerate,
...
void pitch_mark(float_list *input, int length, double samplerate,
  ...
  float_list *tmpf;
  int16_t* waveforms;
  waveforms = new int16_t[length];
  for (i = 0, tmpf = input; tmpf != NULL; ++i, tmpf = tmpf->next) {
    waveforms[i] = tmpf->f;
  }

  EpochTracker et;
  int32_t n_sample = static_cast<int32_t>(length);
  float sample_rate = static_cast<float>(samplerate);
  float min_f0 = static_cast<float>(min);
  float max_f0 = static_cast<float>(max);
  float external_frame_interval = kExternalFrameInterval;
  et.set_unvoiced_cost(threshold);
  bool do_high_pass = true;
  bool do_hilbert_transform = false;
  float inter_pulse = kUnvoicedPulseInterval;
  if (!et.Init(waveforms, n_sample, sample_rate,
  ...
  for (int i = 0; i < pm->num_frames(); ++i) {
    switch (otype) {
      case 1: {
        pitchmark = pm->t(i);
        if (pm->v(i)) {
          fwrite(&pitchmark, sizeof(pitchmark), 1, stdout);
        }
        break;
      }
      case 2: {
      ...
      default: {
        if (pm->v(i)) {
          ...
          fwrite(&polarity, sizeof(polarity), 1, stdout);
        }
        if (i == pm->num_frames() - 1) {
          for (int j = past_pitchmark; j < length; ++j) {
            fwrite(&zero, sizeof(zero), 1, stdout);
          }
        }
        break;
