...
{MLSA digital filter for speech synthesis}{digital filter}
...
This command reads mel-cepstrum coefficients
$c_\alpha(0),c_\alpha(1),\ldots,c_\alpha(M)$ from {\em mcfile},
passes the excitation built from {\em infile} through
the corrsponding MLSA filter, and sends the results to the standard
output.
\par
Input and output data are in float format.
\par
The exponential transfer function $H(z)$ related to the MLSA synthesis filter
is obtained from the $M$ order mel-cepstrum coefficients
...
have same value as the LMA filter (refer to lmadf).
\end{qsection}

\begin{options}
	\argm{m}{M}{order of mel-cepstrum}{25}
	\argm{a}{A}{all-pass constant $\alpha$бе}{0.35}
